...
class Slam:
    def __init__(self, config):
        ...
        if self.do_adaptation and config.depth_pose.batch_size > 1:  # and False:
            # dynamic
            dynamic_replay_buffer_path = config.replay_buffer.dynamic_load_path
            dynamic_replay_buffer_path.mkdir(parents=True, exist_ok=True)
            self.dynamic_replay_buffer = DynamicReplayBuffer(
                dynamic_replay_buffer_path,
                self.online_dataset_type,
                height=self.online_dataset.height,
                width=self.online_dataset.width,
                scales=self.online_dataset.scales,
                frames=self.online_dataset.frame_ids,
                ...
                similarity_threshold=config.replay_buffer.dynamic_similarity_threshold,
                similarity_sampling=config.replay_buffer.similarity_sampling,
            )
            # static
            static_replay_buffer_path = config.replay_buffer.static_load_path
            static_replay_buffer_path.mkdir(parents=True, exist_ok=True)
            static_replay_buffer_path_state_path = static_replay_buffer_path / 'buffer_state.pkl'
            static_replay_buffer_path_state_path = static_replay_buffer_path_state_path if \
                static_replay_buffer_path_state_path.exists() else None
            self.static_replay_buffer = StaticReplayBuffer(
                static_replay_buffer_path,
                self.online_dataset_type,
                static_replay_buffer_path_state_path,
                ...
                similarity_threshold=config.replay_buffer.static_similarity_threshold,
                similarity_sampling=config.replay_buffer.similarity_sampling,
            )
        else:
            self.dynamic_replay_buffer = None
            self.static_replay_buffer = None
    ...
    def step(self):
        ...
        online_image = online_data['rgb', 0, 0]
        online_histogram = histogram(online_image)
        online_phash = pHash(online_image)

        if self.dynamic_replay_buffer is not None:
            self.dynamic_replay_buffer.dynamic_add(online_data,
                                                   self.online_dataset.get_item_filenames(self.current_step - 1),
                                                   self.static_replay_buffer,
                                                   online_histogram,
                                                   verbose=True)
        # =================================================
        if self.dynamic_replay_buffer is not None:
            static_replay_data = self.static_replay_buffer.static_get(online_data, online_phash)
            dynamic_replay_data = self.dynamic_replay_buffer.dynamic_get(online_data, online_histogram)
            if len(static_replay_data) == 0:
                replay_data = self._cat_dict(dynamic_replay_data, dynamic_replay_data)
            else:
                replay_data = self._cat_dict(dynamic_replay_data, static_replay_data)
            if replay_data:
        ...
        if self.do_loop_closures and self.current_step >= self.start_frame:
            ...
            if not self.current_step % self.keyframe_frequency and self.current_step < 4000 and not self.loop_flag:
                if self.since_last_loop_closures > self.lc_distance_poses:
                    ...
                    if len(lc_step_ids) > 0:
                        self.loop_step.append(self.current_step)
                        self.loop_index.append(lc_step_ids)
                        self.loop_distance.append(distances)
                        self.loop_flag = True
                        self.loop_picture += 1
            elif self.loop_flag and self.loop_picture > 0:
                lc_step_ids, distances = self.loop_closure_detection.search(self.current_step)
                if len(lc_step_ids) > 0 and self.loop_picture < 3:
                    self.loop_step.append(self.current_step)
                    self.loop_index.append(lc_step_ids)
                    self.loop_distance.append(distances)
                    self.loop_picture += 1
                    print("loop_picture = ", distances)
                elif self.do_adaptation and self.loop_picture < 2:
                    optimized = True
                else:
                    optimized = True
                    # for index in range(len(self.loop_step)):
                    # graph optimisation
                    for i, d in zip(self.loop_index[0], self.loop_distance[0]):
                        lc_image = self.online_dataset[i - 1]['rgb', 1, 0]
                        lc_transformation, cov_matrix = self.predictor.predict_pose(image,
                                                                                    lc_image,
                                                                                    as_numpy=True)
                        graph_transformation = self.pose_graph.get_transform(self.loop_step[-1], i)
                        print(f'{self.loop_step[-1]} --> {i} '
                              f'[sim={d:.3f}, pred_dist={np.linalg.norm(lc_transformation):.1f}m, '
                              f'graph_dist={np.linalg.norm(graph_transformation):.1f}m]')
                        cov_matrix = np.eye(6)
                        cov_matrix[2, 2] = .1
                        cov_matrix[5, 5] = .1
                        self.pose_graph.add_edge((self.loop_step[-1], i),
                                                 lc_transformation,
    ...
    def save_model(self) -> None:
        ...
        if self.static_replay_buffer is not None:
            self.dynamic_replay_buffer.release_dynamic_buffer(self.static_replay_buffer)
            self.static_replay_buffer.save_state()
